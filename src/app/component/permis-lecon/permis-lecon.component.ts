...
export class PermisLeconComponent implements OnInit {
  ...
  chargerLeconsPassees() {
    const user = JSON.parse(localStorage.getItem('user') || '{}');
    if (!user?.id) return;

    const body = { id_eleve: user.id };

    this.http.post<any[]>('https://forart.alwaysdata.net/get_lecons_conduite_passees.php', body)
      .subscribe(data => {
        this.leconsPassees = data;
      });
  }
